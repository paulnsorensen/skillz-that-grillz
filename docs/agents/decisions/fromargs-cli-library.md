...
- **Decision:** `run()` always writes each error as one stderr line: `{"error": <message>, "exit_code": <n>}`. There is no plain-text error mode. Repair `note:` lines stay plain text on stderr.
...
- **Decision:** `fromargs.App` composes a `cyclopts.App`. Register a command with `@app.command` and a nested group with `app.group(name)`. `App.command` rejects a handler that declares a `json` or `full` parameter, since `run()` owns both. A handler's return value is data, not an exit status: `None` means exit 0 with no stdout, and any other JSON-serializable value is printed as one JSON document. `run()` always exits 0 for a normal return and reports every error through ADR-001's envelope.
...
- **Consequences:** A handler's return value is no longer its exit code; only a raised `CliError` (or `contract_error`) sets a non-zero exit. This is a breaking change from the easy-cheese prototype: an `int` or `bool` return value now serializes as JSON instead of setting the exit status. `App.command(limit=n)` truncates a sequence result and prints a `note:` line on stderr unless `--full` is passed.

_Source: .cheese/age/fromargs-cli-library.md:18-31 and the Cure fixes · Updated: 2026-09-24_

[^1]: lib/fromargs/src/fromargs/_argv.py:26-37; lib/fromargs/tests/test_argv.py:32-48
[^2]: lib/fromargs/src/fromargs/_run.py:90-101
[^3]: lib/fromargs/tests/test_run.py:319-352
