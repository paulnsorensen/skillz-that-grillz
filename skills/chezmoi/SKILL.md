...
|---|---|---|
| `chezmoi.<fmt>` (the *config*) | `$XDG_CONFIG_HOME/chezmoi/chezmoi.toml`, default `~/.config/chezmoi/chezmoi.toml`. **NOT** in the source repo. | Generated by `chezmoi init` (or `apply --init`) from the template below. Per-machine. Holds secrets, API tokens, machine-specific settings — never commit. |
| `.chezmoi.<fmt>.tmpl` (the *config template*) | Source-state root: `~/.local/share/chezmoi/.chezmoi.toml.tmpl`. **Committed** to the dotfiles repo. | You write it. Uses `promptBoolOnce` / `promptChoiceOnce` / `promptStringOnce` so a fresh machine generates the right config on first init. |
