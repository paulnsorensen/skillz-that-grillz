...
| `skills/python-authoring/SKILL.md` | `/python-authoring` | Write or edit Python as concise, stdlib-first, fully typed code, or review it for style and types: validate input once at the boundary, model it with frozen dataclasses, prefer succinct idioms, de-slop, and pass basedpyright with zero errors and warnings. Not for bug or security review. |
...
| `python-authoring` | methodology + `basedpyright` | Python, basedpyright (or `uvx`) | pytest |
